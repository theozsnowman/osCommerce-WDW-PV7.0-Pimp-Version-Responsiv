...
<?php
  for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
  	$product_info_query = tep_db_query("select products_tax_class_id, products_image, image_display, image_folder from " . TABLE_PRODUCTS . " where products_id = '" . (int)$order->products[$i]['id'] . "'");
    ...
    $image_overlay_sales = '';
		...
		if ( tep_not_null(tep_get_products_special_price($order->products[$i]['id'])) ) {
			if ( DISPLAY_OVERLAY_IMAGES_SALES == 'true') {
				$image_overlay_sales = '<div id="wdw_overlay_sale_product_info" class="wdw_overlay_sale_product_info">' . tep_image('includes/languages/' . $_SESSION['language'] . '/images/' . 'overlay-sale.png', IMAGE_SALE, '60', '45', 'style="margin-left: 0px; margin-top: -50px"') . '</div>';
			}
		}
    
    if ($product_info['image_display'] == 1) {
    	$image = tep_image('includes/languages/' . $language . '/images/' . 'no_picture.gif', TEXT_NO_PICTURE, '60', '45', 'style="margin-bottom: 5px;"') . $image_overlay_sales;
    } elseif (($product_info['image_display'] != 2) && tep_not_null($product_info['products_image'])) {
      $image = tep_image(DIR_WS_IMAGES_THUMBS . $product_info['image_folder'] . $product_info['products_image'], $order->products[$i]['name'], '60', '45', 'style="margin-bottom: 5px;"') . $image_overlay_sales;
    }
      
    echo '          <tr>' . "\n" .
 				 '            <td align="left" valign="top" width="60">' . $image . '</td>' . "\n" .
         '            <td align="right" valign="top" width="40">' . $order->products[$i]['qty'] . '&nbsp;x&nbsp;</td>' . "\n" .
    ...
    echo '</td>' . "\n";
    // old code
    //if (sizeof($order->info['tax_groups']) > 1) echo '            <td valign="top" align="right">' . TEXT_VAT . ' ' . tep_display_tax_value($order->products[$i]['tax']) . '%</td>' . "\n";

    echo '            <td align="right" valign="top">' . $currencies->display_price($order->products[$i]['final_price'], $order->products[$i]['tax'], $order->products[$i]['qty']) . $wdw_vat. '</td>' . "\n" .
         '          </tr>' . "\n";
  }
?>
