    ...
    function process() {
      ...
      if (tep_not_null($order->info['shipping_method'])) {
        ...
        if ($GLOBALS[$module]->tax_class > 0) {
        	$wdw_vat = ( DISPLAY_TAX_INFO == 'true' ) ? ( DISPLAY_PRICE_WITH_TAX == 'true' ) ? '<span class="wdw_vat_text">'. sprintf(TEXT_INCL_VAT, tep_get_tax_rate($GLOBALS[$module]->tax_class, $order->delivery['country']['id'], $order->delivery['zone_id']).'%') . '</span>' : '<span class="wdw_vat_text">' . TEXT_EXCL_VAT . '</span>' : '';
        ...
        $this->output[] = array('title' => $order->info['shipping_method'] . ':',
                                'text' => $wdw_vat . ' ' . $currencies->format($order->info['shipping_cost'], true, $order->info['currency'], $order->info['currency_value']),
