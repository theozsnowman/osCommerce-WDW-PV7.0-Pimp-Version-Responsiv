    ...
    function execute() {
      ...
      if ($sp_count > 0) {
        ...
        while ($specials_product = tep_db_fetch_array($sp_query)) {
    			...
    			$wdw_vat = ( DISPLAY_TAX_INFO == 'true' ) ? ( DISPLAY_PRICE_WITH_TAX == 'true' ) ? '<br /><span class="wdw_vat_text">'. sprintf(TEXT_INCL_VAT, tep_get_tax_rate($specials_product['products_tax_class_id']).'%') . '</span>' : '<br /><span class="wdw_vat_text">' . TEXT_EXCL_VAT . '</span>' : '';
