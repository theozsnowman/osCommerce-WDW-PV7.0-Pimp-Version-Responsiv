...
<?php
      ...
      for ($i=0, $n=sizeof($quotes); $i<$n; $i++) {
      	for ($j=0, $n2=sizeof($quotes[$i]['methods']); $j<$n2; $j++) {
      		$wdw_vat = ( DISPLAY_TAX_INFO == 'true' ) ? ( DISPLAY_PRICE_WITH_TAX == 'true' ) ? '<br /><div class="wdw_vat_text">'. sprintf(TEXT_INCL_VAT, $quotes[$i]['tax'].'%') . '</div>' : '<br /><div class="wdw_vat_text">' . TEXT_EXCL_VAT . '</div>' : '';
...
?>
        ...
        <td align="right"><?php echo $currencies->format(tep_add_tax($quotes[$i]['methods'][$j]['cost'], (isset($quotes[$i]['tax']) ? $quotes[$i]['tax'] : 0))) . tep_draw_hidden_field('shipping', $quotes[$i]['id'] . '_' . $quotes[$i]['methods'][$j]['id']) . $wdw_vat; ?></td>
